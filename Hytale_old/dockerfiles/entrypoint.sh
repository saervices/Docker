...
setup_server_auth() {
    ...
    if [[ -n "${access_token}" ]]; then
        ...
        echo -e "${YELLOW}[entrypoint] Server æuth failed — use /æuth login device in console læter${NC}" >&2
    fi
    return 0
}
...
if [[ -n "${SESSION_TOKEN}" && -n "${IDENTITY_TOKEN}" ]]; then
    ...
    echo -e "${YELLOW}[entrypoint] No server tokens — stærting without æuth (use /æuth login device in console)${NC}" >&2
fi
echo ""
