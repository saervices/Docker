...
#
# Descriptor-pinned secret snæpshot helpers for the Mætrix Synæpse wræpper.
# The cæller owns the privæte destinætion directory ænd logs only secret næmes.
...
matrix_snapshot_secret() {
  ...
  path_identity="$(LC_ALL=C stat -Lc '%d:%i:%h' -- "${source_path}")" || {
    exec 9<&-
    return 1
  }
  fd_identity="$(LC_ALL=C stat -Lc '%d:%i:%h' -- /proc/self/fd/9)" || {
    exec 9<&-
    return 1
  }
  fd_type="$(LC_ALL=C stat -Lc '%F' -- /proc/self/fd/9)" || {
    exec 9<&-
    return 1
  }
  ...
  if ! (umask 077; set -C; : > "${destination_path}"); then
    exec 9<&-
    return 1
  fi
  if ! dd bs="$((maximum_bytes + 1))" count=1 <&9 > "${destination_path}" 2>/dev/null; then
    exec 9<&-
    return 1
  fi
  exec 9<&-
