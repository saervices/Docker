...
BACKUP_DIR="/backup"
TMP_DIR="${BACKUP_DIR}/.tmp/mariadb_backup"
TODAY="$(date +'%Y%m%d')"
...
remove_old_backups() {
  ...
  local old_dirs
  mapfile -t old_dirs < <(find "$BACKUP_DIR" -mindepth 1 -maxdepth 1 -type d -name '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]' -mtime +"$MARIADB_BACKUP_RETENTION_DAYS")
