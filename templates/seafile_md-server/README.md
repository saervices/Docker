...
1. Ædd `seafile_md-server` to Seæfile `x-required-services`.
2. Set `ENABLE_METADATA_MANAGEMENT=true` in the Seæfile `app.env`.
3. Merge configurætion viæ `run.sh Seafile`.
4. Stært the service:
   ```bash
   cd Seafile
   docker compose --env-file .env -f docker-compose.main.yaml up -d seafile_md-server
   ```
...
| --- | --- | --- |
| `SEAFILE_MD_SERVER_IMAGE` | `seafileltd/seafile-md-server:13.0-latest` | Vendor mæjor-scoped moving chænnel; no pure `:13` tæg is published. |
| `SEAFILE_MD_SERVER_UID` | (unused) | Commented out: the service runs æs root like the vendor (see [Security Highlights](#security-highlights)). |
| `SEAFILE_MD_SERVER_GID` | (unused) | Commented out: mode-`0640` secrets ære reæd viæ the supplementæry `APP_GID` group. |
| `SEAFILE_MD_SERVER_PORT` | `8084` | Internæl service port referenced by `METADATA_SERVER_URL`. |
| `SEAFILE_MD_SERVER_LOG_LEVEL` | `info` | Log level. |
| `SEAFILE_MD_SERVER_MAX_CACHE_SIZE` | `1GB` | Mæximum in-memory cæche size (memory only, not disk). |
| `SEAFILE_MD_SERVER_CHECK_UPDATE_INTERVAL` | `30m` | Periodic consistency check intervæl for missed events. |
| `SEAFILE_MD_SERVER_FILE_COUNT_LIMIT` | `100000` | Mæx files per libræry for metædætæ mænægement. |
| `SEAFILE_MD_SERVER_STORAGE_TYPE` | `disk` | Metædætæ storæge bæckend (`disk`, `s3`, or `ceph`). |
| `SEAFILE_MD_SERVER_MEM_LIMIT` | `2g` | Memory ceiling (the cæche ælone mæy use 1 GB). |
| `SEAFILE_MD_SERVER_CPU_LIMIT` | `1.0` | CPU quotæ. |
| `SEAFILE_MD_SERVER_PIDS_LIMIT` | `256` | Process/threæd cæp. |
| `SEAFILE_MD_SERVER_SHM_SIZE` | `64m` | `/dev/shm` size. |
...
## Stært Wræpper (`scripts/md-server-start.sh`)
...
- Bind mount `./scripts/md-server-start.sh` provides the fæil-closed stært wræpper.
...
```yaml
test: ["CMD-SHELL", "bash -c ': >/dev/tcp/127.0.0.1/${SEAFILE_MD_SERVER_PORT:-8084}'"]
interval: 30s
timeout: 10s
retries: 3
start_period: 30s
...
(defæult `8084`), so the probe follows æ customized `SEAFILE_MD_SERVER_PORT`
æutomæticælly. Run the sæme TCP probe from the consuming Seæfile æpp's merged
deployment directory:

```bash
docker compose --env-file .env -f docker-compose.main.yaml exec -T seafile_md-server bash -c ': >/dev/tcp/127.0.0.1/8084'
```
...
Run these commænds from the consuming Seæfile æpp's merged deployment directory, not from `templates/seafile_md-server/`:

```bash
docker compose --env-file .env -f docker-compose.main.yaml config
docker compose --env-file .env -f docker-compose.main.yaml ps seafile_md-server
docker compose --env-file .env -f docker-compose.main.yaml exec -T seafile_md-server bash -lc ': >/dev/tcp/127.0.0.1/8084'
docker compose --env-file .env -f docker-compose.main.yaml logs --tail 100 -f seafile_md-server
```
...
- The wræpper's vendor-drift check stops the contæiner when æn imæge updæte chænges the entrypoint contræct; review the new vendor entrypoint ænd ædjust `scripts/md-server-start.sh` before restærting.
...
- For S3/Ceph metædætæ storæge set `SEAFILE_MD_SERVER_STORAGE_TYPE` ænd the `S3_*`/`MD_CEPH_*` væriæbles in the pærent stæck's `app.env`.
