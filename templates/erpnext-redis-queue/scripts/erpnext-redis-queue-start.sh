...
readonly secret_file=/run/secrets/ERPNEXT_REDIS_QUEUE_PASSWORD
readonly vendor_entrypoint=/usr/local/bin/docker-entrypoint.sh
readonly min_secret_bytes=12
readonly max_secret_bytes=4096

fatal() {
  printf '[erpnext-redis-queue] ERROR: %s\n' "$*" >&2
  exit 1
}

if [ -L "$secret_file" ] || [ ! -f "$secret_file" ] || [ ! -r "$secret_file" ]; then
  fatal 'Queue password secret is missing, unreadable, or not a regular file.'
fi

secret_size="$(wc -c < "$secret_file")"
if [ "$secret_size" -eq 9 ] && [ "$(cat "$secret_file")" = 'CHANGE_ME' ]; then
  fatal 'Queue password secret still contains the placeholder value.'
fi
if [ "$secret_size" -lt "$min_secret_bytes" ] || [ "$secret_size" -gt "$max_secret_bytes" ]; then
  fatal 'Queue password secret must contain 12 through 4096 bytes.'
fi
line_free_size="$(LC_ALL=C tr -d '\n\r' < "$secret_file" | wc -c)"
if [ "$line_free_size" -ne "$secret_size" ]; then
  fatal 'Queue password secret contains line breaks.'
fi
if LC_ALL=C grep -q '[[:cntrl:]]' "$secret_file"; then
  fatal 'Queue password secret contains control characters.'
fi

if ! printf '%s\n' "${ERPNEXT_REDIS_QUEUE_MAXMEMORY:-768mb}" | grep -Eq '^[1-9][0-9]*(kb|mb|gb)$'; then
  fatal 'ERPNEXT_REDIS_QUEUE_MAXMEMORY must be a positive kb, mb, or gb value.'
fi
...
runtime_dir="$(mktemp -d /tmp/erpnext-redis-queue.XXXXXX)"
...

{
  ...
  printf 'aof-use-rdb-preamble yes\n'
  printf 'maxmemory %s\n' "${ERPNEXT_REDIS_QUEUE_MAXMEMORY:-768mb}"
