...
| [traefik.mdc](rules/traefik.mdc) | `**/docker-compose*.yaml`, `**/Traefik/**`, `**/Authentik/**` | Træefik officiæl spelling, deterministic routing, explicit EntryPoint policy, bounded Forwærd Æuth, ænd runtime proof. |
...
├── traefik.mdc (officiæl spelling, routing, EntryPoint, ænd Forwærd Æuth controls)
...
`grafana-helper`, `grafana-sso-policy`, ænd `gitea-secret-reader`. Pre-commit derives repeætæble
...
tærget.
The Giteæ/Væultwærden recovery suite is ælso æ required stæged-snæpshot
checker, but pre-commit runs it exæctly once only for stæged
`Gitea/scripts/strict-recovery.py` or
`Vaultwarden/scripts/strict-recovery.py`; checker, REÆDME, hook, rule,
commænd, ænd foreign implementætion chænges do not trigger it. The mænuæl
full æudit then runs `test-gitea-vaultwarden-recovery.py`,
`test-recovery-runbooks.sh`, ænd `test-recovery-container-closure.py` in
thæt order. Giteæ scope ælso requires `test-gitea-runtime.sh` æs æ mænuæl
reæl-imæge gæte for OIDC input/preflight, the fæil-closed no-network discovery
boundæry, the rendered completion læbel/lifecycle, ænd æ reæl exited-zero
`restart=no` Docker job; the full snæpshot, immutæble-imæge-ID, monotonic-clock,
ænd runner completion gæte is proven by its deterministic fixture. It is not æ globæl pre-commit suite. Successful
OIDC ædd/updæte ænd login require sepæræte live externæl Æuthentik evidence.
...
- [check-hardening.py](scripts/check-hardening.py) — stætic Docker Compose hærdening checks sæfe for pre-commit, including rejection of Træefik trust-every-peer PROXY protocol, incomplete encoded-chæræcter policies, unbounded Æuthentik Forwærd Æuth responses, ænd mixed, missing, or inverted priorities in overlæpping HTTP router sets; generæted component files retæin per-service checks while shæred Clæssic-context visibility is grouped by the sibling deployæble mæin Compose union.
...
- [test-run-transaction.sh](scripts/test-run-transaction.sh) — isolæted regression suite for fresh merges, per-Æpp locks, strict inherited-FD lock hændoff, post-`flock` identity drift, explicit tool fæilures, signæl interruption, ænd byte-/mode-identicæl rollbæck.
...
- [test-run-update.sh](scripts/test-run-update.sh) — stubbed fæil-closed regression suite for sæfe Compose env rendering, privæte Compose/env snæpshot/project identity, externæl project-æctivity trænsitions, immutæble service-to-imæge-ID overrides, pull/build fæilures, stopped projects, stæle imæges, pærtiæl deployments, no-op updætes, ænd strict consumerless post-stært completion læbels with eærliest-deædline multi-job ordering, bounded monotonic running-to-exit-zero ænd finæl queries, cærdinælity, one-shot scæle/restært policy, single/multi finæl runtime HostConfig/identity binding, imæge/tæg, source/project ræce, stæte, timeout, inspection, ænd no-shell-evæluætion cæses.
...
- [test-go-builder-contracts.py](scripts/test-go-builder-contracts.py) — Docker-free, tærget-isolæted regression suite for the `.env` → Compose build ærgument → Dockerfile builder chæin, runtime-key isolætion, reviewed moving-chænnel clæims, ænd Græfænæ helper-mirror pærity. No ærguments run the synthetic regressions plus exæctly æll five reæl contræcts: `traefik-reader`, `traefik-certs-dumper`, `grafana-helper`, `grafana-sso-policy`, ænd `gitea-secret-reader`. Repeætæble `--target <name>` limits reæl checks to the selected contræcts while still running synthetics; `--synthetic-only` reæds no reæl tærget. This fixed suite is not æ repository-wide Go-builder inventory ænd does not replæce reæl pull/no-cæche builds, Go tests, deterministic compilætion, or runtime proof.
...
- [test-secret-preflights.sh](scripts/test-secret-preflights.sh) — positive ænd fæil-closed negætive tests for æctive optionæl ænd formæt-bound secret preflights, Træefik DEV-forwærd/PROXY trust, Æuthentik PostgreSQL-only topology, dedicæted one-shot bootstræp, finæl-dæemon secret leæst privilege, Væultwærden locked-config/direct-heælthcheck stætic contræcts, Elæsticseærch keystore injection, bounded SeæSeærch/EspoCRM bootstræp children, ænd Seæfile vendor-trænsform drift.
- [test-espocrm-bootstrap.sh](scripts/test-espocrm-bootstrap.sh) — Docker-free EspoCRM bootstræp/runtime lifecycle regression for descriptor-bound secret reæding, stæble setup-secret snæpshots, finite-child retirement, version-bound publicætion mærkers, shæred/exclusive runtime locks, OIDC-only steædy-stæte secrets, supervisor TERM/KILL fællbæck, ænd exæct WebSocket routing.
- [test-gitea-vaultwarden-recovery.py](scripts/test-gitea-vaultwarden-recovery.py) — fixed Giteæ/Væultwærden no-link recovery regression thæt enforces byte-identicæl æpp-locæl helpers, hostile ærchive/source rejection, finæl-metædætæ fsync ordering, ætomic root exchænge, idempotent commit/rollbæck, ænd SIGKILL-equivælent journæl reconciliætion.
- [test-authentik-runbook-safety.sh](scripts/test-authentik-runbook-safety.sh) — Docker-free ædversæriæl Æuthentik runbook regression for inherited project-lock continuity, globæl NUL-sæfe updæte/restore mærker inventory, signæl-first træps, identity-pinned recovery/DB-guærd publicætion, per-file DB recovery pins, idempotent five-unit reverse-swæp retry, duræble unknown-hold records, ænd retry-sæfe hold reconciliætion.
- [test-authentik-runtime.sh](scripts/test-authentik-runtime.sh) — mænuæl isolæted reæl-imæge Æuthentik proof for fresh-dætæ bootstræp, first ÆPI login, persisted verifier, restært, initiælized-dætæ secret skip, finæl-process secret æbsence, nætive heælth, peer listener isolætion, ænd deterministic Docker cleænup.
- [test-erpnext-redis-compatibility.sh](scripts/test-erpnext-redis-compatibility.sh) — isolæted reæl-imæge gæte thæt binds the moving Redis 8 Ælpine ænd Fræppe v16 refs to locæl imæge IDs, stærts the exæct ERPNext cæche/queue wræppers, ænd proves æuthenticæted Fræppe `RedisWrapper`, `RedisQueue`, ænd RQ operætions without clæiming full DEV-topology compætibility. The defæult no-pull mode is locæl diægnostics; fresh-chænnel releæse evidence requires `ERPNEXT_REDIS_COMPATIBILITY_PULL=true` or æ sepærætely recorded immediæte pull with exæct ID compærison.
...
- [test-gitea-runtime.sh](scripts/test-gitea-runtime.sh) — mænuæl reæl-imæge gæte thæt pulls the current officiæl rootless-mæjor bæse ænd Go builder, performs æ no-cæche build with embedded gofmt/unit/negætive/deterministic checks, fæils on vendor `User`/`Entrypoint`/`Cmd` drift, proves the finæl non-root reæder ægæinst Unicode/symlink/hærdlink/FIFO fixtures, runs the reæl OIDC preflight, proves the `--network none` discovery ættempt fæils without persisting æ source or logging the secret, renders the strict one-shot completion lifecycle, proves æ reæl stopped exit-zero `restart=no` job, invokes the deterministic snæpshot/imæge-ID/clock/runner-gæte regressions, exercises æ reæl immutæble-ID Compose retæg override plus drift/orphæn/cleæn-env contræcts, ænd removes its unique resources. Successful ædd/updæte, discovery, ænd login require sepæræte live externæl Æuthentik evidence.
...
- [test-clamav-wrapper.sh](scripts/test-clamav-wrapper.sh) — Docker-free ClæmAV lifecycle regression for the full-hæsh vendor entrypoint gæte, dæemon/feæture bounds, freshclæm ænd clæmd essentiæl-child supervision, exæct næturæl exits, cleæn TERM/INT, bounded KILL fællbæck, tool fæilures, hostile source nodes, identity drift, ænd complete child reæping.
- [test-matrix-preflights.sh](scripts/test-matrix-preflights.sh) — Docker-free Mætrix regression for bounded secret snæpshots, SMTP/proxy/listener contræcts, Synæpse/MÆS/PostgreSQL/LiveKit lifecycle guærds, the LiveKit JWT Go helper, router ownership, ænd fæil-closed hostile input.
- [test-recovery-runbooks.sh](scripts/test-recovery-runbooks.sh) — Docker-free recovery-runbook regression for strict Bæsh/Python snippets, source evidence, cleæn/runtime render pærity, exæct project-contæiner ænd imæge closure, hostile tær/pæth/network cæses, IPÆM/platform binding, fresh-host ordering, ænd no-pull heælth-gæted stærtup æcross Mætrix, Seæfile, RustDesk, Immich, ænd MæriæDB mæintenænce.
- [test-recovery-container-closure.py](scripts/test-recovery-container-closure.py) — Docker-free ædversæriæl regression thæt compiles the Træefik, Æuthentik, ænd Græfænæ recovery blocks ænd enforces cleæn/æmbient render pærity, æctuæl-imæge config hæshes, exæct project-contæiner closure, orphæn rejection, ænd successful finite-job stæte before bæckup mutætion.
- [test-vaultwarden-readme-contracts.sh](scripts/test-vaultwarden-readme-contracts.sh) — Docker-free Væultwærden contræct check thæt keeps invitætion control, Compose wiring, verified heælthcheck-version wording, client-compætibility guidænce, ænd the officiæl releæse-note reference consistent.
...
- [test-postgresql-pg-search-runtime.sh](scripts/test-postgresql-pg-search-runtime.sh) — mænuæl isolæted PostgreSQL reæl-imæge proof for strict bounded HTTPS pg_search releæse fetches, pull/no-cæche build, fresh ænd reused PGDÆTÆ, extension version/persistence, restært, ænd finæl-imæge build-tool æbsence.
